...
- **`/explore` has two entry points.** Clean island/type landings are rewritten to `/explore/f/[[...facet]]` (ISR, 1h); narrower queries use the dynamic `page.tsx` route. Both server-render the first 24 cards through `server/listings-search.ts#getExploreData`, which does not cache booking eligibility across requests. Filters are shared by the pages, `/api/listings` and `/api/listings/count` through `lib/listing-filters.ts`. Clean landings with a confirmed zero count get `noindex, follow`; populated landings stay indexable, and a failed data load must not be mistaken for an empty category. Narrower queries remain `noindex` with a canonical back to their island/type landing.
