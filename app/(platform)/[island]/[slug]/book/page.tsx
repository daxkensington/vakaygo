...
export default function BookingPage() {
  ...
  const current = useCurrentBookingEligibility(listing?.id);
  const [error,setError] = useState("");
  useEffect(() => {
    const abort = new AbortController();
    fetch("/api/listings/"+encodeURIComponent(slug),{signal:abort.signal, cache:"no-store"})
  ...
  return <><Header /><main className="min-h-screen bg-cream-50 px-6 pt-24 pb-12"><div className="max-w-xl mx-auto">
    ...
    <h1 className="text-2xl font-bold text-navy-700 my-6">{listing ? listing.title : "Listing information"}</h1>
    {error ? <p role="alert">{error}</p> : listing ? <BookingWidget listing={listing} bookingEligible={current.eligible} unclaimed={listing.typeData?.unclaimed === true} /> : <p role="status">Loading listing…</p>}
  </div></main></>;
}
